...
export default function PassengerPage() {
  ...
  const sessionId = useRef(`yatri-${Math.random().toString(36).slice(2, 10)}`)
  ...
  // Re-derive the status card whenever the selection (or snapshot) changes.
  useEffect(() => {
    if (!state || !selected) return
    const train = state.trains.find((t) => t.number === selected)
    setLive(train ? initialLive(train, state.sim_time) : null)
  ...
  const sendText = async () => {
    ...
    setBusy(true)
    try {
      const { reply } = await postChat(message, sessionId.current)
  ...
  const onAudio = useCallback(async (blob: Blob) => {
    ...
    setBusy(true)
    try {
      const res = await postVoice(blob, sessionId.current)
