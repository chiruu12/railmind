...
The frontend reaches the backend through `frontend/src/lib/apiBase.ts`
(`API_BASE` from `VITE_API_BASE`) for HTTP and `frontend/src/api/ws.ts`
(`VITE_WS_URL`) for the WebSocket.

## 1. Backend → Render

Render deploys from a Git repo via the `render.yaml` Blueprint at the repo root.

1. Push this branch to GitHub (already done once the PR merges to `main`).
