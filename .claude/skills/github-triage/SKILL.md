...
|----------|-------------|
| "This issue is clear enough, skip the /domain-model session" | Issues that seem clear often have hidden scope. The grilling session surfaces edge cases and constraints that prevent rejected PRs and wasted dev time. |
| "Let's just mark it ready-for-agent, the agent will figure it out" | Agents without clear briefs produce wrong solutions. A 10-minute grilling session saves hours of agent rework. The brief IS the value. |
| "The reporter didn't provide enough info, just close it" | Move to needs-info first. Reporters may have valuable bugs but poor reporting skills. Give them one chance with specific questions before closing. |
| "I'll skip the out-of-scope check, this looks new" | Previously rejected concepts resurface under different names. The .out-of-scope/ check takes 30 seconds and prevents re-litigating settled decisions. |
| "No need for the AI disclaimer, it's obvious from context" | Every GitHub comment must include the disclaimer. Transparency is non-negotiable — readers months later won't have context. |

## Red Flags

- Issues accumulating in needs-triage with no movement — triage cadence is too slow
- Agent briefs that lack acceptance criteria — agents can't verify their own work
- needs-info issues sitting without follow-up for 2+ weeks — should be closed or re-evaluated
- Multiple conflicting state labels on a single issue — state machine violated

## Verification Checklist

- [ ] Every label transition follows the state machine (no skipped states without override)
- [ ] AI disclaimer present on every GitHub comment
- [ ] Agent briefs include acceptance criteria and scope boundaries
- [ ] needs-info comments have specific, actionable questions (not "please provide more info")
- [ ] .out-of-scope/ check done before triaging enhancement requests
- [ ] Bug reproduction attempted before starting /domain-model session
- [ ] Each issue has exactly one state label and one category label

## Anti-patterns

- **DO NOT** skip the AI disclaimer on any GitHub comment — transparency is mandatory
- **DO NOT** mark issues ready-for-agent without a clear agent brief — agents need specifications, not hopes
- **DO NOT** close issues without explanation — always post a comment explaining why
- **DO NOT** re-ask questions that were resolved in previous triage sessions — read prior triage notes first
- **DO NOT** apply conflicting state labels — one state label per issue, always
